...
Due to these reporting inaccuracies and regulatory violations, I am requesting the permanent deletion of this tradeline from my credit report.`;
  ...
  if (!hasAnyNegative && !showPositiveDetails && !expandAll && !showAllDetails) {
    ...
    return (
      <Card className="border-gray-200 bg-white cursor-pointer hover:bg-gray-50 transition-colors rounded-lg" onClick={() => setShowPositiveDetails(true)}>
  ...
  if (isCollapsed && isDisputeSaved) {
    return (
      ...
      >
        <CardContent className="px-4 md:px-6 py-4">
          <div
            className="flex-between cursor-pointer hover:bg-green-100 -mx-4 md:-mx-6 -my-4 px-4 md:px-6 py-4 rounded-lg transition-colors"
              ...
              <div>
                ...
                <p className="text-sm text-green-600 font-medium">
                  {(() => {
                    ...
                    // Only count bureaus that are specifically marked as negative
                    let disputeCount = 0;

                    // Check TransUnion - only count if explicitly marked as negative
                    if (transUnionStatus === 'Negative') disputeCount++;

                    // Check Equifax - only count if explicitly marked as negative
                    if (equifaxStatus === 'Negative') disputeCount++;

                    // Check Experian - only count if explicitly marked as negative
                    if (experianStatus === 'Negative') disputeCount++;
    ...
    >
      <CardContent className={`px-4 md:px-6 ${hasAnyNegative ? 'pt-6 pb-6' : 'pt-1 pb-2'}`}>
        ...
        {!hasAnyNegative && (showPositiveDetails || expandAll) && (
          <div className="flex-between mb-4 -mx-2 mt-6">
            <div></div>
            {!expandAll && (
              <button
        ...
        <div className="mb-4">
          ...
          <div className="hidden md:block relative">
            {/* Invisible clickable area above bureau headers - expanded to full width and height */}
            {!hasAnyNegative && showPositiveDetails && !expandAll && (
              <div
                className="absolute -top-12 -left-6 -right-6 h-16 cursor-pointer hover:bg-gray-100 hover:bg-opacity-30 rounded-t-lg transition-colors z-10"
                onClick={() => setShowPositiveDetails(false)}
              />
            )}
        ...
        {/* Show All Info Toggle Button - Always visible unless individual card is collapsed */}
