...
export function PersonalInfo({
  ...
  const proceedWithSave = () => {
    ...
    // Notify parent component that dispute was saved - THIS WAS MISSING!
    if (onDisputeSaved) {
      onDisputeSaved({
        reason: finalReason,
    ...
    // New choreography: green → scroll to 20px above section → pause 1/2 second → collapse → wait 1 second → scroll to next section
    console.log('Personal info dispute saved, immediately starting choreography');

    // Immediately scroll to 20 pixels above "Personal Information" section
    ...
    if (personalInfoSection) {
      ...
      // After scroll completes, pause 1/2 second then collapse the section
      setTimeout(() => {
        console.log('300ms scroll wait complete, now pausing 500ms before collapse');
        setTimeout(() => {
          console.log('500ms pause complete, NOW COLLAPSING personal info section');
          // Collapse handled by parent component

          // Wait 1 second after collapse, then scroll to 20px above Hard Inquiries
          setTimeout(() => {
        ...
        }, 500); // Pause 1/2 second before collapse
      }, 300); // Wait for scroll to complete
    } else {
      // Fallback - collapse handled by parent
      setTimeout(() => {
        const nextSection = document.querySelector('[data-section="inquiries"]');
        if (nextSection) {
          const rect = nextSection.getBoundingClientRect();
  ...
  // No collapsed state needed here - the header handles collapsed view
    ...
    <div data-section="personal-info">
      ...
      >
        ...
        {isExpanded && (
          ...
          <div className="flex items-start justify-between gap-2 mb-4 bg-white">
            {/* Clickable area for Show Less functionality - ends 10px before button */}
            <div 
              className={`flex items-start gap-2 flex-1 cursor-pointer transition-colors rounded-md ${
                isDisputeSaved ? 'hover:bg-green-100' : hasSelectedItems ? 'hover:bg-red-75' : 'hover:bg-gray-50'
              } ${!isDisputeSaved ? 'md:mr-[220px]' : ''}`}
              onClick={
                  isDisputeSaved
                    ? () => {
          ...
          <div className={`${hasSelectedItems && !isDisputeSaved ? 'bg-rose-50 border border-rose-200' : 'bg-white border border-gray-300'} rounded-lg p-6`}>
              {/* Header with controlled clickable area */}
                ...
                <div 
                  ...
                  <div className="flex justify-between items-start w-full">
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-base ${isDisputeSaved ? 'text-green-700' : 'text-gray-700'}`}>
                        <strong>
                          {isDisputeSaved ? (
                    ...
                    {/* Select All Previous Addresses Button - Right-aligned in pink area */}
                    {!isDisputeSaved && (
                      <Button
                        size="sm"
                        className="bg-gray-100 hover:bg-gray-200 text-gray-700 border border-gray-300 h-8 text-sm font-medium flex-shrink-0"
                        onClick={(e) => {
                          ...
                          // Wait for expansion to take effect, then select addresses and scroll
                          setTimeout(
                            () => {
                              const newSelections = { ...selectedItems };

                              // Select previous addresses for all three bureaus
                              const bureaus = ['transunion', 'equifax', 'experian'];

                              // Find previous address in all items (expanded list)
                              ...
                              if (previousAddressItem) {
                                bureaus.forEach((bureau) => {
                                  const itemId = `${bureau}-${previousAddressItem.id}`;
                                  newSelections[itemId] = true;
                                });

                                setSelectedItems(newSelections);
                                console.log('Selected all previous addresses:', newSelections);

                                // Trigger auto-typing effect after selecting all previous addresses
                                if (!isDisputeSaved && !selectedReason && !selectedInstruction) {
                                  console.log('Triggering auto-typing effect from Select All button');

                                  // Clear any existing selections and mark as AI generated
                                  setSelectedReason('');
                                  setSelectedInstruction('');
                                  setShowCustomReason(false);
                                  setShowCustomInstruction(false);
                                  ...
                                  setTimeout(async () => {
                                    ...
                                    // Show arrow after typing completes
                                    setTimeout(() => {
                                      console.log('About to show arrow after Select All auto-typing');
                                      checkFormCompletionAndShowArrowWithValues(
                                        newSelections,
                                        reasonText,
                                        instructionText
                                ...
                                console.log('No previous address items found');
                              }

                              // Auto-scroll to 20px above Former Names/Aliases box
                              setTimeout(() => {
                                // Find the Former Names/Aliases box in the Personal Information section
                                const formerNamesElement = document.querySelector(
                                  '[data-field="former-names"]'
                                );
